...
export default async function ProductPage({
  ...
  return (
    <main className="mx-auto min-h-[70vh] w-full max-w-7xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
      <nav className="mb-6 flex items-center gap-2 text-sm text-muted-2" aria-label="Breadcrumb">
        <Link href="/" className="transition hover:text-primary-bright">
          Home
        </Link>
        <span aria-hidden="true">/</span>
        <Link href="/products" className="transition hover:text-primary-bright">
          Products
        </Link>
        <span aria-hidden="true">/</span>
        <Link
      ...
      <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <Link
              href={`/products?category=${product.category.slug}`}
              className="rounded-full bg-primary-soft px-3 py-1 text-xs font-semibold uppercase tracking-widest text-primary transition hover:bg-primary-soft"
            >
              {product.category.name}
            </Link>
            {product.featured && (
          ...
          <h1 className="mt-4 text-3xl font-bold text-ink sm:text-4xl">{product.name}</h1>
          ...
          {product.farmer && (
            <p className="mt-2 text-sm text-muted-2">
              Grown and packed by{" "}
              <span className="font-semibold text-primary-bright">{product.farmer.name}</span>
          ...
          <div className="mt-5 flex items-end gap-3">
            ...
            <p className="pb-1 text-sm text-muted-2">per {product.unit}</p>
          </div>
          ...
          <div className="mt-4 flex items-center gap-2 text-sm">
            <span
              className={`h-2 w-2 rounded-full ${inStock ? "bg-primary-solid" : "bg-red-500"}`}
              aria-hidden="true"
            />
            <span className={inStock ? "font-medium text-primary-bright" : "font-medium text-red-500"}>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-line bg-surface p-4 text-center">
              <p className="text-xs font-medium text-muted-2">Freshness</p>
              <p className="mt-1 text-sm font-semibold text-ink">Farm to home in 24h</p>
            </div>
            <div className="rounded-xl border border-line bg-surface p-4 text-center">
              <p className="text-xs font-medium text-muted-2">Delivery</p>
              <p className="mt-1 text-sm font-semibold text-ink">Free above ₹499</p>
            </div>
            <div className="rounded-xl border border-line bg-surface p-4 text-center">
              <p className="text-xs font-medium text-muted-2">Returns</p>
              <p className="mt-1 text-sm font-semibold text-ink">Easy 7-day policy</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="mt-16" aria-labelledby="reviews-heading">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 id="reviews-heading" className="mt-1 text-2xl font-bold text-ink">
              What customers say
            </h2>
          </div>
          {rating.count > 0 && (
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            {reviews.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-line-strong py-12 text-center text-muted-2">
            ...
            {user ? (
              ...
              <div className="rounded-2xl border border-line bg-surface p-5 text-center shadow-sm">
                <p className="text-sm text-muted">
                  <Link href="/login" className="font-semibold text-primary-bright hover:underline">
                    Log in
                  </Link>{" "}
        ...
        <section className="mt-16">
          <div className="mb-6 flex items-end justify-between">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-bold text-ink">More from {product.category.name}</h2>
            </div>
            <Link href="/products" className="text-sm font-medium text-primary-bright hover:underline">
              View all
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-4">
            {related.map((item) => (
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="truncate text-sm font-semibold text-ink group-hover:text-primary-bright">
                    {item.name}
                  </h3>
                  <p className="mt-0.5 text-xs text-muted-2">{item.unit}</p>
                  {(() => {
                  ...
                  <p className="mt-2 text-base font-bold text-primary">{formatPrice(item.price)}</p>
                </div>
              </Link>
            ))}
          </div>
